...
class MaxEnt(object):
    def __init__(self):
        self.feats = defaultdict(int)  #特征feature字典
        self.trainset = []   # 一个数组 存放训练集
        self.labels = set()  #set和其他语言类似, 是一个无序不重复元素集, 基本功能包括关系测试和消除重复元素
#
    ...
    def load_data(self,file):
        for line in open(file):
            ...
            # the first column is label  第一列作为标签
            label = fields[0]
